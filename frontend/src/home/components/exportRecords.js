...
const jsonToCsv = (data) => {
  let csv = 'id,名称,类别,资产编号,购入时间,使用状态,是否领用,用户id,领用者,领用时间,位置,配置\n';
  const records = equiFormat(data);
  records.forEach(item => {
    const newConf = item.configuration.replace(/\r/g, '')
    const stringItem = `${item.id + '\t'},${item.name + '\t'},${item.category + '\t'},${item.number + '\t'},${formatDate(item.buy_time) + '\t'},${item.state + '\t'},${item.is_receive + '\t'},${item.user_id + '\t'},${item.username + '\t'},${formatDate(item.receive_time) + '\t'},${item.location + '\t'},${newConf + '\t'}\n`;
    csv += stringItem;
  })
  return csv;
}
